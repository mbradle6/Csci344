...
function tweetsToimgURL(){
   return statuses.map(tweet=> tweet.image_url
    ).filter(img_url => img_url !== undefined);
    //returns list of strings (img urls)

}
const images = tweetsToimgURL();
console.log(images);


function getInfluencers(){
    return (statuses.filter(tweet => tweet.retweet_count>= 500
        ).map(tweet => tweet.screen_name));
}
console.log(getInfluencers());

const mostPopular = (tweetA,tweetB) =>{
    
 //   return (tweetA.retweet_count>tweetB.retweet_count) ? tweetA : tweetB ;
}
function getMostpopular(){
    return (statuses.reduce(mostPopular));
}
